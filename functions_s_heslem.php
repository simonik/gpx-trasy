...
function preved($text) { # prevede znaky pro zvyrazneni textu, smajly, odkazy
    $text = preg_replace("'\[b\]([^\[\]<>]+)\[\/b\]'si", "<strong>\\1</strong>", $text);  # formatovaci znacky
    $text = preg_replace("'\[i\]([^\[\]<>]+)\[\/i\]'si", "<cite>\\1</cite>", $text); 
    $text = preg_replace("'\[u\]([^\[\]<>]+)\[\/u\]'si", "<ins>\\1</ins>", $text); 
   // $text = preg_replace('#(http://|ftp://|(www\.))([\w\-]*\.[\w\-\.]*([/?][^\s]*)?)#e', "'<a rel=\"nofollow\" href=\"'.('\\1'=='www.'?'http://':'\\1').'\\2\\3\">'.((strlen('\\2\\3')>23)?(substr('\\2\\3',0,20).'...'):'\\2\\3').'</a>'",$text); # prevod odkazu
	$text=preg_replace('#(https?://|ftp://|(www\.))([\w\-]*\.[\w\-\.]*([/?][^\s]*)?([ \n\r\t\.,/\(\):;\-\\\/\?!\"\\\'\[\]\{\}=]*))|((mailto:)?([^\s]+@[\w\-]+\.[\w]+))#e',"prevod('\\1','\\2','\\3','\\8')",$text);	
    
    $text = str_replace(array("\n", "&"), array("<br />", "&amp;"), $text);               # novy radek, osetreni ampersandu  
    $text = ereg_replace("(\*\*)+([0-9]{1,2})", "<img src='akce/kniha/files/images/x\\2.gif' width='15' height='15' alt='\\2' />", $text);  # smajlici
    $text = ereg_replace("&amp;(lt;|gt;|amp;|quot;)", "&\\1", $text);                     # nektere entity jsou povolene
    return $text;
}

function preved_web($web) { # prevede odkazy v poli web zadane pouze ve formatu www
    return eregi("^(www\..+\..{2,3})$", $web) ? $web = eregi_replace("^(www\..+\..{2,3})$", "http://\\1", $web) : $web = $web;
}
    ...
    $_POST['jmeno']=$_SESSION["jmeno"];
      ...
      //  $datum = strtr($datum, array('+01+' => 'Ledna', '+02+' => 'Února', '+03+' => 'Bøezna', '+04+' => 'Dubna', '+05+' => 'Kvìtna', '+06+' => 'Èervna', '+07+' => 'Èervence', '+08+' => 'Srpna', '+09+' => 'Záøí', '+10+' => 'Øíjna', '+11+' => 'Listopadu', '+12+' => 'Prosince'));
       $datum = Date("Y-m-d H:i" ); 
        ...
        for ($i = 0; $i <= $n - 1; $i++) {           // zkontrolujeme každé slovo
    $zprava_array_lower = strtolower($zprava_array[$i]);
    if (in_array($zprava_array_lower, $spamwords_array, true)) {  // našli jsme spam-slovo
        // return $message = "Komentáø nebyl uložen…";  // zakomentováno – už neskonèíme pøedèasnì
        // pøípadnì sem mùžete dát $zprava_return .= '[SPAM?] ';  // oznaèení slova, ale necháme text dál
    }
    // slova delší než 50 znakù zalomíme pomlèkou
    $zprava_array[$i] = wordwrap($zprava_array[$i], 50, "-", 1);
